<?php
$adfly	=	get_option('wall_adfly');
$go = $_GET['go'];
if (!empty($go))
{
$url = base64_decode($go);
$url = 'http://adf.ly/'.$adfly.'/'.$url;
...
?>
<?php
$keyw   	=	str_replace(' ','+',get_option('wall_keyword'));
...
if ($page !=1)
   ...
   echo '<div class="pagination"><a href="/?page='.$before.'" class="inactive"> Previous Page</a><span class="current">Page '.$page.'</span><a href="/?page='.$next.'" class="inactive">Next Page </a></div>';
   }
else
{
 $next = $page + 1;
 echo '<div class="pagination"><span class="current">Page '.$page.'</span><a href="/?page='.$next.'" class="inactive">Next Page </a></div>';
}
?>
